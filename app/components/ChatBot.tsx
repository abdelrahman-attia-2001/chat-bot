...
export default function ChatBot() {
  ...
  const sendMessage = async (message: string) => {
    ...
    const aboutPatterns = ["الموقع", "عن الموقع", "who made this site", "what is this site"];
    ...
    setTimeout(() => {
      setMessages((prev) => [...prev, { role: "assistant", content: "This is a dummy AI reply." }]);
      setLoading(false);
    }, 1000);
  };
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {messages.length === 0 ? (
                ...
                messages.map((msg, i) => (
                  ...
                  >
                    {msg.role === "assistant" && <LuBot className="text-cyan-400 mt-1 mr-2" size={22} />}
                    <div
                    ...
                    {msg.role === "user" && <FaUser className="text-gray-400 ml-2 mt-1" size={22} />}
                  </div>
                ))
              )}
            ...
            {/* Input */}
            <form onSubmit={handleSubmit} className="flex items-center border-t border-gray-700 bg-zinc-900 relative">
              <input
                type="text"
                value={input}
              ...
              <button type="submit" disabled={!input.trim()} className="p-3 text-white bg-cyan-600 hover:bg-cyan-700 transition">
                <FiSend size={18} />
              </button>
            </form>

            <button onClick={() => setIsOpen(false)} className="absolute top-3 right-3 text-gray-400 hover:text-white transition">
